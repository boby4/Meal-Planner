...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await env.DB.prepare(
      `SELECT id, recipe_name, recipe_data, source, viewed_at FROM history WHERE ${clause} ORDER BY viewed_at DESC LIMIT ?`
...
export async function POST(request: NextRequest) {
  try {
    ...
    ).bind(userId, userId ? "" : deviceId, recipe_name, recipe_data ? JSON.stringify(recipe_data) : null, source || "").run();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await env.DB.prepare(`DELETE FROM history WHERE ${clause}`).bind(...binds).run();
    return NextResponse.json({ success: true });
  } catch (error) {
